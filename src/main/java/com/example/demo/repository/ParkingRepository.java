...
public interface ParkingRepository extends JpaRepository<Parking,Integer> {
    ...
    @Query(value = "select parkingid,user.username,car.carlicense,user.phone,entrancetime from car,user,parking where parking.carlicense = car.carlicense " +
    ...
    @Query(value = "select car.carid,car.carlicense,0 as exist," +
            "(select exittime from parkrecord,car where car.CarId=parkrecord.carid and car.carLicense = ?1 ORDER BY exittime desc limit 1) as time" +
            " from car where car.carlicense = ?1",nativeQuery = true)
    CarDetail_devtool getCarDetailByWx_out (String carlicense);
    @Query(value = "select car.carid,car.carlicense,COUNT(parkingid) as exist," +
            "parking.entrancetime as time from parking,car where car.carlicense=parking.carlicense and car.carlicense = ?1 ORDER BY entrancetime desc limit 1",nativeQuery = true)
    CarDetail_devtool getCarDetailByWx_in (String carlicense);

    Boolean existsByCarlicense(String carlicense);
